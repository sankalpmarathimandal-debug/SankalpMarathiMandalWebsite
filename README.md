...
### ⚠️ The "automatic" GitHub Action referenced below doesn't exist yet

Further down, this README describes a GitHub Action (`.github/workflows/update-logo-variants.yml`) that's supposed to auto-regenerate `data/logo-variants.json` (Community Pride Wall) and `data/highlights.json` (homepage slider) whenever images are added or removed. As of this writing, `.github/workflows/` doesn't exist in this repo — that automation was never actually added, despite being documented as if it were live. Until someone adds it, both files need to be regenerated by hand after adding/removing images:
...
then commit the updated JSON files. (`Start Local Preview.command` already runs `generate_highlights.py` automatically for local preview — but that only affects your local copy, not the live site.)
...
- **Add the `.github/workflows/update-logo-variants.yml` automation** referenced in this README, or update the README to stop claiming it exists — see "Handoff guide" above. Until then, regenerate `data/logo-variants.json` and `data/highlights.json` manually (commands in that section) after adding/removing images.
